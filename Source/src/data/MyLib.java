...
public class MyLib {
    ...
    private static HashMap<String, Connection> conncheck = new HashMap<String, Connection>();
...
//        return con;
        ...
        if(!conncheck.containsKey("connection"))
        {
        	try {           
		...
		/*
		 * try { //String url = "jdbc:sqlserver://" + ServerName + ":" + Port +
		 * ";databasename=" + DatabaseName; String url = "jdbc:postgresql://" +
		 * ServerName + ":" + Port + "/" + DatabaseName; con =
		 * DriverManager.getConnection(url, UserName, Password); } catch (SQLException
		 * ex) { Logger.getLogger(MyLib.class.getName()).log(Level.SEVERE, null, ex); }
		 */
    }
